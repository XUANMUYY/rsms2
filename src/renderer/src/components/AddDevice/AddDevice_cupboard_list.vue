<template>
  <v-card class="mx-auto"
    ...
    <v-card-actions>
      ...
      <v-spacer></v-spacer>
      <v-btn :loading="loading" :disabled="useAddDeviceStore().cupbox_id ==''" :color="color" variant="flat" @click="Submit">{{ BtnTips }}</v-btn>
    </v-card-actions>
  </v-card>
</template>
...
import AddSource_cupboard_list_box from './AddDevice_cupboard_list_box.vue'
...
function Submit() {
  loading.value = true
  setTimeout(()=>{
    useAddDeviceStore().Add_cupboard_list().then((resolve)=>{
      if (resolve[0].affectedRows==1){
        useAddDeviceStore().step = 3
      }else {
        color.value = "red"
...
function Close() {
  useBoardTCPStore().CreatSocket(0, useAddDeviceStore().device_list_data.wiz_ip as string, useAddDeviceStore().device_list_data.wiz_port as number)
  CloseLoading.value = true
  useBoardTCPStore().FormatBoard(0).then((resolve)=>{
    if(resolve){
      useAddDeviceStore().Remove_device_list().then((resolve)=>{
        if(resolve[0].affectedRows==1){
